...
public class ZeroToEmptyConverter : IValueConverter
{
    ...
    public object ConvertBack(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        ...
        {
            if (targetType == typeof(uint))  return (uint)0;
            if (targetType == typeof(double)) return (double)0;
            return 0;
        }

        if (targetType == typeof(uint) && uint.TryParse(s, out uint u))  return u;
        if (targetType == typeof(double) && double.TryParse(s, NumberStyles.Any, culture, out double d)) return d;
